...
This version handles only start/stop commands for systemd services
"""
import logging
import os
import json
...
def setup_logging(debug=False, verbose=False, log_file: Optional[str] = None):
    ...
    return logging.getLogger("StartStopController")
...
class StartStopController:
    ...
    # This controller handles only start/stop commands
    ...
    def connect_mqtt(self) -> mqtt_client.Client:
        ...
        def on_message(client, userdata, msg):
            ...
            # Handle service state changes for start/stop commands only
                ...
                if msg_json.get('service') and msg_json.get('state') not in ['start', 'stop', 'started', 'stopped']:
                    pass
                else:
...
def main():
    ...
    try:
        controller = StartStopController(debug, log_file)
        controller.run()
    except Exception as e:
